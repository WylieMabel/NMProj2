...
(The centered and scaled values are computed as xstd = (x − x ̄)/σx with x ̄ the average and σx the standard deviation of the simulated distribution).
...
```
Only 3 statistics in the observed network lie within the 95 CI (names 003, 030T, 030C), suggesting the model has a very poor fit.
Additionally, the p-value in iv) is 0 so we can reject the null hypothesis that the manhobis difference is the same between the two. MORE INTERPRETATION
